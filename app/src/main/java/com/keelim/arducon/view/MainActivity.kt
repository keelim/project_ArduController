...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navigationView.setNavigationItemSelectedListener { menuItem: MenuItem ->
            when (menuItem.itemId) {
                ...
                R.id.drawer_bug_report -> {
                    startActivity(Intent(this, WebViewActivity::class.java))
                    Snackbar.make(drawerlayout, "기능 준비 중입니다", Snackbar.LENGTH_SHORT).show()
                }
                R.id.drawer_setting -> {
                    startActivity(Intent(this, SettingActivity::class.java))
                    Snackbar.make(drawerlayout, "기능 준비 중입니다", Snackbar.LENGTH_SHORT).show()
                }
                R.id.drawer_controller -> {
    ...
    private fun connectToSelectedDevice(selectedDeviceName: String) { // BluetoothDevice 원격 블루투스 기기를 나타냄.
         ...
         */
        val mRemoteDevice = getDeviceFromBondedList(selectedDeviceName)
        ...
        mWorkerThread = Thread(Runnable {
            ...
            while (!Thread.currentThread().isInterrupted) {
                try { // InputStream.available() : 다른 스레드에서 blocking 하기 전까지 읽은 수 있는 문자열 개수를 반환함.
                    ...
                    Toast.makeText(applicationContext, "데이터 수신 중 오류가 발생 했습니다.", Toast.LENGTH_LONG).show()
                }
            }
        })
    ...
    private fun selectDevice() {
        ...
        pariedDeviceCount = mDevices.size
        ...
        if (pariedDeviceCount == 0) // 페어링된 장치가 없는 경우.
            ...
            val items = arrayOfNulls<String>(listItems.size)
        ...
        if (requestCode == 1) {
            when (resultCode) {
                ...
                Activity.RESULT_CANCELED ->
                    Toast.makeText(applicationContext, "블루투스를 사용할 수 없어 프로그램을 종료합니다", Toast.LENGTH_LONG).show()
            }
        }
