		...
		setContentView(R.layout.activity_proximity_sensor);
		 ...
		 tVProximity = findViewById(R.id.tVProximity);
		  ...
		  if (proximitySensor == null){
		   Toast.makeText(ProximitySensorActivity.this,"No Proximity Sensor Found! ",Toast.LENGTH_LONG).show();
		  }
		 }
		 ...
		 public void onSensorChanged(SensorEvent event) {
		  ...
		  if(event.sensor.getType()==Sensor.TYPE_PROXIMITY){
		   if(event.values[0]==0){
		    tVProximity.setText("You are Near: "+ event.values[0]);
		   }
		   else{
		    tVProximity.setText("You are Far: "+ event.values[0]);
		   }
